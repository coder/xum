#!/usr/bin/env bash
# Fail when a tracked *.test.ts(x) file is run by no CI lane, or by two.
#
...
#      `jest tests/`).
#   3. The explicit list below: files run some other way, or deliberately not in CI.
set -euo pipefail
...
git ls-files --cached --others --exclude-standard -- '*.test.ts' '*.test.tsx' | LC_ALL=C sort -u >"$tmp/all"
./scripts/test-unit-ci.sh --list-all | sed 's#^\./##' | LC_ALL=C sort >"$tmp/unit"
if ! bun x jest --listTests >"$tmp/jest.raw" 2>"$tmp/jest.err"; then
  cat "$tmp/jest.err" >&2
  exit 1
fi
sed "s#^$PWD/##" "$tmp/jest.raw" | { grep '^tests/' || true; } | LC_ALL=C sort >"$tmp/jest"
...
LC_ALL=C sort -u "$tmp/unit" "$tmp/jest" "$tmp/other" >"$tmp/routed"
...
LC_ALL=C sort "$tmp/unit" "$tmp/jest" "$tmp/other" | uniq -d >"$tmp/duplicated"
...
if ((status == 0)); then
  echo "check-test-routing: $(wc -l <"$tmp/all") test files routed ($(wc -l <"$tmp/unit") bun unit, $(wc -l <"$tmp/jest") jest, $(wc -l <"$tmp/other") other)"
fi
exit "$status"
