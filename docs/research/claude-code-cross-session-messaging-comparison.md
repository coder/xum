...
- `task(kind="workspace", workspace.mode="existing")` continues a turn in an existing top-level workspace, but **only one the caller itself created**: `TaskService.createWorkspaceTurn` requires a durable `WorkspaceTurnTaskHandleRecord` with `createdWorkspace: true` matching the target (`src/node/services/taskHandleStore.ts`, persisted under `~/.mux/sessions/<owner>/task-handles/`). The only other accepted target is a descendant sub-agent workspace via the internal `allowAgentWorkspace` flag (the reactivation path above — not exposed in the tool schema). Arbitrary user workspaces return `invalid_scope`.
