    ...
    scope: z
      ...
      .describe(
        'Listing scope. "descendants" (default) lists this workspace\'s own tasks, workflow runs, and bash processes. "tree" lists every agent workspace in this task tree — ancestors, siblings/cousins, descendants, and the root workspace row (status "workspace") — each tagged with its relationship to you; use it to discover task_send_message peer targets. ' +
          '"instance" is the on-demand address book for local/worktree callers: eligible local/worktree root workspaces across projects in this Xum instance (never another tree\'s sub-agents), newest first, paged with `limit`/`offset` and narrowed with `query`; each row carries `projectPath`, an `activity` snapshot, and its relationship to you (self, ancestor, or unrelated).'
      ),
    query: z
      .string()
      .nullish()
...
export const TOOL_DEFINITIONS = {
  ...
  task_list: {
    ...
    description:
      ...
      'Pass scope:"instance" from a local/worktree workspace for the on-demand address book of this Xum instance: eligible local/worktree root workspaces across projects (status "workspace", never another tree\'s sub-agents), tagged self, ancestor, or unrelated, ordered newest first by createdAt. Narrow with `query` (ID, title, name, project path), page with `limit`/`offset`, and continue from `nextOffset` when it is returned; `activity` (busy/idle) is a snapshot taken at listing time. Rows are addressable via task_send_message — unrelated targets receive your text as an untrusted agent message, queued to turn-end while they are busy, under their own agent/model settings; discovery grants no additional control, and existing ownership rights remain unchanged. ' +
