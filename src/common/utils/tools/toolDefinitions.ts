...
export const TaskSendMessageToolArgsSchema = z
  .object({
    task_id: z
      ...
      .min(1)
      .describe(
        'Target workspace ID: a descendant sub-agent task ID returned by task, a same-tree row from task_list scope:"tree" (peer, ancestor, or root), or any other workspace ID in this Xum instance that you already know — an envelope "from" reply address or an ID the user provided. Unrelated (cross-tree) targets may be root workspaces or live sub-agents of other trees.'
      ),
    message: z
      .string()
      .trim()
