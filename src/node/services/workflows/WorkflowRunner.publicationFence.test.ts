...
/**
 * Reproducers for coder/xum #4452 gap 2 (red by design, evidence only).
 *
 ...
 *
 * Runner A's lease renewals are stalled (a stalled holder); B runs on a clock 60 s ahead so A's
 * lease reads as stale to B without real waits.
 */
...
describe("WorkflowRunner child publication after a lease takeover (#4452 gap 2)", () => {
  ...
  test("finding (green on main): another backend's recovery refuses to replace A's checkpointed, unpublished child", async () => {
  ...
  test("RED on main (latent): if B's recovery may replace the unpublished child, A's unfenced commit publishes a second child", async () => {
    const { storeA, storeB } = await createStores();
    const published: string[] = [];
    const a = createBackendTaskService({ name: "a", published, holdCommit: true });
    const b = createBackendTaskService({
  ...
  test("RED on main (compound with gap 1): a checkpoint stalled inside its owner-checked write lands after B published", async () => {
    const { storeA, storeB } = await createStores();
    const published: string[] = [];
    const otherProcessTaskService = createOtherProcessTaskService(
      fixture.config,
    ...
    const a = createBackendTaskService({
      ...
      beforeReservationCallbacks: () => {
        ...
        // owner check: A stalls there holding events.jsonl.lock and lease.json.lock.
    ...
    // A stalls past the mkdir-lock reclaim window (it never refreshes the lock mtimes).
    const runDir = path.join(sessionDir, "workflows", RUN_ID);
    const staleSeconds = (Date.now() - 5_000) / 1_000;
    for (const lockDir of ["events.jsonl.lock", "lease.json.lock"]) {
      await fs.utimes(path.join(runDir, lockDir), staleSeconds, staleSeconds);
    }

    // B takes over; the journal has no checkpoint for the step, so B reserves and publishes.
    const runB = settle(runnerB.run(RUN_ID));
    await b.backend.published;
    const journalBeforeResume = startedTaskIdForStep(await storeB.getRun(RUN_ID));

    pauseA?.release();
    storeA.unstallRenewals();
    const runAOutcome = await runA;
    const journalAfterResume = startedTaskIdForStep(await storeB.getRun(RUN_ID));
    b.backend.releaseReports();
    const runBOutcome = await runB;

    expect(journalBeforeResume).toBe("task_b_1_0");
    expect(runAOutcome.kind).toBe("rejected");
    // SAFE outcome: one published child, the journal keeps naming it, and B finishes with it.
    expect({
      published,
      journalAfterResume,
      runB:
        runBOutcome.kind === "resolved"
          ? runBOutcome.value
          : `rejected: ${runBOutcome.error instanceof Error ? runBOutcome.error.message : String(runBOutcome.error)}`,
    }).toEqual({
      published: ["task_b_1_0"],
      journalAfterResume: "task_b_1_0",
      runB: { reportMarkdown: "Final: report from task_b_1_0" },
    });
  });
});
