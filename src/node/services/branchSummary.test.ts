...
function meatyExchange(idPrefix: string): XumMessage[] {
  const filler = `investigated the flaky ${idPrefix} test and traced the race `.repeat(200);
  return [
    createXumMessage(`${idPrefix}-user`, "user", `Please fix this: ${filler}`, { timestamp: 1 }),
    createXumMessage(`${idPrefix}-assistant`, "assistant", `Findings: ${filler}`, {
      timestamp: 2,
    }),
  ];
}
...
describe("buildAbandonedBranchTranscript", () => {
  ...
  test("clamps a single message that exceeds the transcript cap, keeping the tail", () => {
    const oversized = createXumMessage(
      "big-1",
      "user",
...
describe("maybeAppendAbandonedBranchSummary", () => {
  ...
  test("tiny abandoned segments skip the model call", async () => {
    const { historyService, cleanup } = await createTestHistoryService();
    try {
      const tiny = [createXumMessage("tiny-user", "user", "one line", { timestamp: 1 })];
      const appended = await maybeAppendAbandonedBranchSummary({
        historyService,
        aiService: unreachableAiService(),
        workspaceId: "ws-tiny",
  ...
  test("preserved-tail copies and compaction rows are excluded from the summarizer input", async () => {
    ...
    try {
      ...
      const compactionRow = createXumMessage(
        "compact-1",
        "assistant",
  ...
  test("tail guard drops the summary when history advanced past the branch point", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("bp-1", "assistant", "branch point", { timestamp: 1 });
      ...
      const firstTurn = createXumMessage("u-1", "user", "already moved on", { timestamp: 2 });
...
describe("branch summary placement on fork/truncate flows", () => {
  test("fork-from-message: summary row lands at the end of the new branch before any next request", async () => {
    ...
    try {
      ...
      const firstSend = createXumMessage("m3", "user", "continuing on the fork", { timestamp: 5 });
  ...
  test("a send in another process waits on the pending marker before proceeding (r48)", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("xp-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
      ...
      await startAbandonedBranchSummaryInBackground({
        ...
        sessionDir,
        abandonedMessages: [
          createXumMessage("xp-abandoned-user", "user", `Fix this: ${filler}`, { timestamp: 2 }),
          createXumMessage("xp-abandoned-assistant", "assistant", `Findings: ${filler}`, {
            timestamp: 3,
          }),
        ],
        experiments: RLM_ON,
  ...
  test("summary that settles before the first send stays consumable", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("sb-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
  ...
  test("concurrent first sends both wait so the summary lands before either appends", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("cc-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
      ...
      const sendUser = async (id: string) => {
        ...
        const append = await historyService.appendToHistory(
          ws,
          createXumMessage(id, "user", `send ${id}`, { timestamp: Date.now() })
        );
  ...
  test("clearPendingBranchSummary drops a registration a removed workspace never consumed", async () => {
    ...
    try {
      const ws = "ws-cleared";
      const branchPoint = createXumMessage("cl-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
  ...
  test("clearPendingBranchSummary invalidates an in-flight writer so it never appends", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("inv-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
  ...
  test("removal during a first-send await still cancels the writer", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("ar-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
  ...
  test("clearPendingBranchSummary waits for an in-flight append before resolving", async () => {
    ...
    try {
      ...
      const branchPoint = createXumMessage("ser-1", "assistant", "branch point", { timestamp: 1 });
      expect((await historyService.appendToHistory(ws, branchPoint)).success).toBe(true);
  ...
  test("edit-resend truncation: summary row precedes the re-sent user message", async () => {
    ...
    try {
      const ws = "ws-edit";
      const kept = [
        createXumMessage("e1", "user", "first question", { timestamp: 1 }),
        createXumMessage("e2", "assistant", "first answer", { timestamp: 2 }),
      ];
      ...
      const editedUser = createXumMessage("e3", "user", "second, better question", {
        timestamp: 3,
      });
