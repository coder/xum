...
- The envelope's "from" id is the reply address for all three relationships: answer with task_send_message when a reply is useful. Any envelope sender is a valid reply address; delivery still follows the target's lifecycle rules (busy targets queue, stopped/archived targets refuse, a root inside a delegated turn asks you to retry later).
</agent-peer-messages>
</prelude>
`;
