...
export class TaskService implements AgentTaskIntegration {
  ...
  private async startReservedAgentTask(plan: TaskLaunchPlan): Promise<void> {
    ...
    // Reuse is tracked explicitly (main #4387): owner-derived shared paths can change during
    // launch (a parent rename re-derives them), so path equality is not a reuse test.
    const sharesParentCheckout = taskWasShared;
    assert(
      !taskWasShared || materialized.reusedExistingCheckout,
