...
interface Fixture extends Disposable {
  ...
  modelCalls: string[];
  emittedMessages: XumMessage[];
  seedTrajectory: (lines?: string[]) => Promise<void>;
  readChat: () => Promise<XumMessage[]>;
...
async function createFixture(options?: {
  ...
  const emittedMessages: XumMessage[] = [];
    ...
    seedTrajectory: async (lines) => {
      ...
      for (const [index, text] of texts.entries()) {
        await historyService.appendToHistory(
          WORKSPACE_ID,
          createXumMessage(`user-${index}`, "user", text, { timestamp: Date.now() })
        );
      }
    },
    readChat: async () => {
...
describe("RefineService", () => {
  ...
  it("stages a memory edit, applies it only on approval, and rolls back via r6", async () => {
    ...
    // (XumMessage role maps 1:1 into the provider request).
  ...
  it("confines the refine input to the active context segment (r37)", async () => {
    ...
    await fixture.historyService.appendToHistory(
      WORKSPACE_ID,
      createXumMessage("reset-boundary-1", "assistant", "", {
        timestamp: now,
    ...
    await fixture.historyService.appendToHistory(
      WORKSPACE_ID,
      createXumMessage("post-reset-user-1", "user", "POST-RESET evidence about the repo.", {
        timestamp: now + 1,
      })
    );
  ...
  it("refuses to apply a proposal staged before a context reset (r37)", async () => {
    ...
    await fixture.historyService.appendToHistory(
      WORKSPACE_ID,
      createXumMessage("reset-boundary-2", "assistant", "", {
        timestamp: Date.now(),
        contextBoundaryKind: CONTEXT_BOUNDARY_KINDS.RESET,
      })
    );
  ...
  it("refuses to publish a proposal when the context was reset mid-pass (r38)", async () => {
    ...
    appendBoundaryOnce = async () => {
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("reset-mid-pass-1", "assistant", "", {
          timestamp: Date.now(),
          contextBoundaryKind: CONTEXT_BOUNDARY_KINDS.RESET,
        })
      );
    ...
    rewriteTailOnce = async () => {
      ...
      for (const id of ["user-1-rewrite", "user-2-rewrite"]) {
        const appended = await fixture.historyService.appendToHistory(
          WORKSPACE_ID,
          createXumMessage(id, "user", `rewritten branch ${id}`, { timestamp: Date.now() })
        );
  ...
  it("fails closed on ambiguous timeline boundaries (r38)", async () => {
...

    {
      ...
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("reset-no-ts", "assistant", "", {
          contextBoundaryKind: CONTEXT_BOUNDARY_KINDS.RESET,
        })
      );
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("post-reset-user-2", "user", "POST-RESET evidence.", {
          timestamp: now + 1,
        })
      );
      expect((await fixture.service.run(WORKSPACE_ID)).success).toBe(true);
...

    {
      ...
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("reset-same-ms", "assistant", "", {
          timestamp: now,
          contextBoundaryKind: CONTEXT_BOUNDARY_KINDS.RESET,
        })
      );
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("post-reset-user-3", "user", "POST-RESET evidence.", {
          timestamp: now + 1,
        })
      );
      expect((await fixture.service.run(WORKSPACE_ID)).success).toBe(true);
...

    {
      ...
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("reset-negative-ts", "assistant", "", {
          timestamp: -1,
          contextBoundaryKind: CONTEXT_BOUNDARY_KINDS.RESET,
        })
      );
      await fixture.historyService.appendToHistory(
        WORKSPACE_ID,
        createXumMessage("post-reset-user-4", "user", "POST-RESET evidence.", {
          timestamp: now + 1,
        })
      );
      expect((await fixture.service.run(WORKSPACE_ID)).success).toBe(true);
