...
import { estimateXumMessageTokens } from "@/common/utils/messages/keepRecentTail";
...
function extractTextForTranscript(message: XumMessage): string {
 ...
 */
function formatMessageForBranchTranscript(message: XumMessage): string {
 ...
 */
export function buildAbandonedBranchTranscript(messages: XumMessage[]): string {
...
export function createBranchSummaryMessage(summaryText: string): XumMessage {
...
export async function maybeAppendAbandonedBranchSummary(
  ...
  try {
    ...
    const estimatedTokens = abandonedMessages.reduce(
      (sum, message) => sum + estimateXumMessageTokens(message),
      0
    );
...
interface PendingBranchSummary {
  promise: Promise<XumMessage | null>;
...
export async function startAbandonedBranchSummaryInBackground(
  ...
  const promise = (async (): Promise<XumMessage | null> => {
