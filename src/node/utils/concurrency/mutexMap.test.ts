...
describe("MutexMap", () => {
  ...
  test("withLockBounded gives up its place at the deadline without disturbing the holder", async () => {
    const mutex = new MutexMap<string>();
    let releaseHolder!: () => void;
    const holderGate = new Promise<void>((resolve) => {
      releaseHolder = resolve;
    });
    const holder = mutex.withLock("key", () => holderGate);
    let abandonedRan = false;

    const timedOut = await mutex.withLockBounded(
      "key",
      () => {
        abandonedRan = true;
    ...
    // The holder still has the key.
    const whileHeld = await mutex.tryWithLock("key", () => Promise.resolve("stolen"));

    releaseHolder();
    await holder;
    const after = await mutex.withLockBounded(
      "key",
      () => Promise.resolve("next"),
      Date.now() + 1_000
    );
    // An uncontended key is entered even with a deadline already past.
    const pastDeadline = await mutex.withLockBounded(
      "free",
      () => Promise.resolve("free"),
      Date.now() - 1
    );

    expect({ timedOut, abandonedRan, whileHeld, after, pastDeadline }).toEqual({
      timedOut: { kind: "timeout" },
      abandonedRan: false,
      whileHeld: { acquired: false },
      after: { kind: "ok", value: "next" },
      pastDeadline: { kind: "ok", value: "free" },
    });
  });

  test("tryWithLock refuses without queueing while the key is held or awaited", async () => {
    const mutex = new MutexMap<string>();
    let releaseHolder!: () => void;
    const holderGate = new Promise<void>((resolve) => {
      releaseHolder = resolve;
    ...
    const holder = mutex.withLock("key", () => holderGate);
    // A queued waiter also counts: a try must not jump ahead of it.
    const waiter = mutex.withLock("key", () => Promise.resolve("waited"));
    let ran = false;

    const refused = await mutex.tryWithLock("key", () => {
      ran = true;
      return Promise.resolve("tried");
    });
    // Other keys stay independent.
    const otherKey = await mutex.tryWithLock("other", () => Promise.resolve("other"));

    releaseHolder();
    await holder;
    expect(await waiter).toBe("waited");
    const acquired = await mutex.tryWithLock("key", () => Promise.resolve("tried"));

    expect({ refused, ran, otherKey, acquired }).toEqual({
      refused: { acquired: false },
      ran: false,
      otherKey: { acquired: true, value: "other" },
      acquired: { acquired: true, value: "tried" },
    });
  });
});
