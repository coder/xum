...
describe("McpIconRefCache", () => {
  ...
  test("a superseded in-flight completion cannot overwrite or evict newer entries", async () => {
    const cache = new McpIconRefCache(1);
    const slow = deferred<string | null>();
    const stale = cache.resolve(ref(1), () => slow.promise);
    cache.reset();
    await cache.resolve(ref(2), () => Promise.resolve(PNG));
    slow.resolve("data:image/png;base64,old=");
    expect(await stale).toBe("data:image/png;base64,old=");
    expect(cache.peek(ref(1))).toBeUndefined();
    expect(cache.peek(ref(2))).toBe(PNG);
  });
});
